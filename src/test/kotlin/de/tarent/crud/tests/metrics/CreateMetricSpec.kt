...
class CreateMetricSpec : BaseMetricSpec() {
    @Test
    fun `create an temperature metric`() =
        spec.componentSpec {
    ...
    @Test
    fun `create an electricity metric`() =
        spec.componentSpec {
            // given: A watt metric
    ...
    @Test
    fun `create metric has further links`() =
        spec.componentSpec {
            // given: A watt metric
            val body = metricJson("W", 64.2, timestamp)
    ...
    private suspend fun ApplicationTestBuilder.postMetric(
        url: String,
        body: String,
    ): HttpResponse =
        client.post(url) {
