...
class UpdateGroupSpec : BaseGroupSpec() {
    @Test
    fun `Update an existing group`() =
        spec.componentSpec {
            // when: We update the group
            val response =
                client.put("/groups/$DEFAULT_GROUP_NAME") {
    ...
    @Test
    fun `Overwrite an existing group id`() =
        spec.componentSpec {
            // when: We update the group
            var response =
                client.put("/groups/$DEFAULT_GROUP_NAME") {
    ...
    @Test
    fun `Failed - group unknown`() =
        spec.componentSpec {
            // when: An unknown group is updated
            val response =
                client.put("/groups/UNKNOWN") {
                    contentType(ContentType.Application.Json)
                    accept(ContentType.Application.Json)
                    setBody(groupJson(DEFAULT_GROUP_NAME, "New description"))
                }
