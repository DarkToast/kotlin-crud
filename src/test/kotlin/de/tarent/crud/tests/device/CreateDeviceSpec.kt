...
class CreateDeviceSpec : BaseDeviceSpec() {
    ...
    fun `Create a device`() =
        spec.componentSpec {
            ...
            // when: We make a POST
            val response =
                client.post("/groups/$testGroupName/devices") {
    ...
    @Test
    fun `devices with same name but in other groups are ok`() =
        spec.componentSpec {
    ...
    @Test
    fun `Failed creation - wrong group`() =
        spec.componentSpec {
            // when: We make a post on an unknown group
            val response =
                client.post("/groups/unknown/devices") {
                    contentType(ContentType.Application.Json)
                    accept(ContentType.Application.Json)
                    setBody(deviceJson)
                }
